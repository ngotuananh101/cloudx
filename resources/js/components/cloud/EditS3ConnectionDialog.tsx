            ...
            <DialogContent className="max-h-[90vh] sm:max-w-2xl overflow-y-auto rounded-3xl p-0 shadow-2xl bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 [&>button]:right-6 [&>button]:top-6 [&>button]:z-10">
                <form onSubmit={submit} className="p-6">
                    ...
                    <div className="mt-5 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
                        ...
                        {showAdvanced && (
                            <div id="edit-s3-advanced-settings" className="grid gap-4 border-t border-gray-100 dark:border-gray-800 p-4 sm:grid-cols-2">
                    ...
                    <div className="mt-6 flex justify-end gap-2">
                        <button type="button" onClick={onClose} disabled={processing} className="rounded-md border border-gray-200 dark:border-gray-700 px-4 py-2 text-sm font-bold text-gray-600 dark:text-gray-400 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-60">Cancel</button>
                        <button type="submit" disabled={processing} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-blue-700 disabled:opacity-60">{processing ? 'Testing connection...' : 'Save changes'}</button>
                    </div>
                </form>
            </DialogContent>
        </Dialog>
...
function Field({ label, error, required = false, children }: { label: string; error?: string; required?: boolean; children: React.ReactNode; }) {
    ...
    return (
        <div className="space-y-2">
            ...
            {children}
            {error && <p id={id ? `${id}-error` : undefined} className="text-xs text-red-600 dark:text-red-400">{error}</p>}
        </div>
    );
}
    ...
    return (
        <div className="space-y-2">
            ...
            <label className="flex h-10 items-center gap-3 rounded-md border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 px-3 text-sm font-bold text-gray-700 dark:text-gray-300">
                <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4 rounded border-gray-300 dark:border-gray-700 text-blue-600 focus:ring-blue-500" />
                {label}
            </label>
