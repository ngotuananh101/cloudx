...
export default function ConnectionNavItem({
    ...
    return (
        <li>
            <div
                className={`group relative flex items-center justify-between gap-2 rounded-lg px-3 py-1.5 text-xs font-bold tracking-wide transition-colors ${isActive ? 'bg-red-50 text-brand' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
            >
                {isActive && (
                ...
                >
                    {connection.provider_icon?.endsWith('.svg') ? (
                        ...
                        <Cloud
                            className={`h-4.5 w-4.5 shrink-0 ${isActive ? 'text-brand' : 'text-gray-400 group-hover:text-gray-600'}`}
                        />
                    )}
                    <span
