...
it('returns only directories', function () {
    ...
    $mockBrowser->shouldReceive('list')
        ->once()
        ->with(Mockery::type(CloudConnection::class), 'test_path')
        ->andReturn([
            new CloudFileData('1', 'test_path/file.txt', 'file.txt', 'file', 100, null, false),
            new CloudFileData('2', 'test_path/folder1', 'folder1', 'folder', 0, null, true),
            new CloudFileData('3', 'test_path/folder2', 'folder2', 'folder', 0, null, true),
        ]);
